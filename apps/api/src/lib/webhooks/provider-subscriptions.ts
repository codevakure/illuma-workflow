...
import { validateAlphanumericId, validateAirtableId } from '@/lib/core/security/input-validation'
import { refreshAccessTokenIfNeeded, getOAuthToken } from '@/lib/auth/oauth-utils'
...
function getNotificationUrl(webhook: any): string {
  ...
  return `${baseUrl}/api/webhooks/trigger/${webhook.path}`
}
 ...
 * Does not fail webhook deletion if cleanup fails.
 */
...
/**
 * Create an Airtable webhook subscription.
 * Throws errors with friendly messages if webhook creation fails.
 */
export async function createAirtableWebhookSubscription(
  ...
  try {
    const { path, providerConfig } = webhook
    ...
    const accessToken = await getOAuthToken(workflow.userId, 'airtable')
    if (!accessToken) {
      airtableLogger.warn(
        `[${requestId}] Could not retrieve Airtable access token for user ${workflow.userId}. Cannot create webhook in Airtable.`
      )
      throw new Error(
    ...
    if (!airtableResponse.ok || responseBody.error) {
      ...
      airtableLogger.error(
        `[${requestId}] Failed to create webhook in Airtable for webhook ${webhook.id}. Status: ${airtableResponse.status}`,
    ...
    airtableLogger.info(
      `[${requestId}] Successfully created webhook in Airtable for webhook ${webhook.id}.`,
      { airtableWebhookId: responseBody.id }
    )
    return { externalId: responseBody.id }
  } catch (error: any) {
    airtableLogger.error(
      `[${requestId}] Exception during Airtable webhook creation for webhook ${webhook.id}.`,
      ...
      try {
        ...
        if (listResp.ok && listBody && Array.isArray(listBody.webhooks)) {
          ...
          airtableLogger.warn(
            ...
            { baseId, status: listResp.status, body: listBody }
          )
        }
      } catch (e: any) {
  ...
  try {
    ...
    if (!calendlyResponse.ok) {
      ...
      calendlyLogger.error(
        `[${requestId}] Failed to create webhook in Calendly for webhook ${webhook.id}. Status: ${calendlyResponse.status}`,
        { response: errorBody }
      )
    ...
    if (!webhookUri) {
      calendlyLogger.error(
        `[${requestId}] Calendly webhook created but no webhook URI returned for webhook ${webhook.id}`,
        { response: responseBody }
      )
    ...
    calendlyLogger.info(
      `[${requestId}] Successfully created webhook in Calendly for webhook ${webhook.id}.`,
      { calendlyWebhookUri: webhookUri, calendlyWebhookId: webhookId }
    )
    return { externalId: webhookId }
  } catch (error: any) {
    calendlyLogger.error(
      `[${requestId}] Exception during Calendly webhook creation for webhook ${webhook.id}.`,
      { message: error.message, stack: error.stack }
    )
    throw error
  }
...
/**
 * Create a Grain webhook subscription.
 * Throws errors with friendly messages if webhook creation fails.
 */
export async function createGrainWebhookSubscription(
  ...
  try {
    ...
    grainLogger.info(
      ...
      { grainWebhookId: responseBody.id, eventTypes }
    )
...
/**
 * Create a Lemlist webhook subscription.
 * Throws errors with friendly messages if webhook creation fails.
 */
...
/**
 * Create a Webflow webhook subscription.
 * Throws errors with friendly messages if webhook creation fails.
 */
export async function createWebflowWebhookSubscription(
  ...
  try {
    ...
    if (!accessToken) {
      webflowLogger.warn(
        `[${requestId}] Could not retrieve Webflow access token for user ${workflow.userId}. Cannot create webhook in Webflow.`
      )
      throw new Error(
    ...
    if (formName && webflowTriggerType === 'form_submission') {
      requestBody.filter = { name: formName }
    }
    ...
    if (!webflowResponse.ok || responseBody.error) {
      ...
      webflowLogger.error(
        `[${requestId}] Failed to create webhook in Webflow for webhook ${webhook.id}. Status: ${webflowResponse.status}`,
        { message: errorMessage, response: responseBody }
      )
    ...
    webflowLogger.info(
      `[${requestId}] Successfully created webhook in Webflow for webhook ${webhook.id}.`,
      { webflowWebhookId: responseBody.id || responseBody._id }
    )

    return { externalId: responseBody.id || responseBody._id }
  } catch (error: any) {
    webflowLogger.error(
      `[${requestId}] Exception during Webflow webhook creation for webhook ${webhook.id}.`,
      { message: error.message, stack: error.stack }
    )
    throw error
  }
...
/**
 * Determines whether an external webhook subscription needs to be recreated.
 */
...
/**
 * Create external webhook subscriptions for the given provider.
 * Dispatches to the appropriate create function based on provider type.
 */
export async function createExternalWebhookSubscription(
  ...
  if (provider === 'airtable') {
    const result = await createAirtableWebhookSubscription(webhook, workflow, requestId)
    if (result) {
      updatedProviderConfig = { ...updatedProviderConfig, ...result }
      externalSubscriptionCreated = true
    }
  } else if (provider === 'calendly') {
    const result = await createCalendlyWebhookSubscription(webhook, workflow, requestId)
    if (result) {
      updatedProviderConfig = { ...updatedProviderConfig, ...result }
      externalSubscriptionCreated = true
    }
  } else if (provider === 'microsoft-teams') {
    const subscriptionId = await createTeamsSubscription(webhook, workflow, requestId)
    if (subscriptionId) {
      updatedProviderConfig = { ...updatedProviderConfig, externalSubscriptionId: subscriptionId }
      externalSubscriptionCreated = true
    }
  } else if (provider === 'telegram') {
    await createTelegramWebhook(webhook, workflow, requestId)
    externalSubscriptionCreated = true
  } else if (provider === 'webflow') {
    const result = await createWebflowWebhookSubscription(webhook, workflow, requestId)
    if (result) {
      updatedProviderConfig = { ...updatedProviderConfig, ...result }
      externalSubscriptionCreated = true
    }
  } else if (provider === 'typeform') {
    const usedTag = await createTypeformWebhook(webhook, requestId)
    ...
    const externalId = await createGrainWebhookSubscription(webhook, requestId)
    if (externalId) {
      updatedProviderConfig = { ...updatedProviderConfig, externalId }
      externalSubscriptionCreated = true
    }
  } else if (provider === 'lemlist') {
    const externalId = await createLemlistWebhookSubscription(webhook, requestId)
    if (externalId) {
      updatedProviderConfig = { ...updatedProviderConfig, externalId }
      externalSubscriptionCreated = true
    }
 ...
 * Handles Airtable, Teams, Telegram, Typeform, Calendly, Grain, Lemlist, and Webflow cleanup.
 * Does not fail deletion if cleanup fails.
 */
