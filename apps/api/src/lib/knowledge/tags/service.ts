/**
 * Tags service for knowledge base tag definitions.
 */
...
export async function getNextAvailableSlot(
  ...
  fieldType: string,
  existingBySlot?: Map<string, unknown>
): Promise<string | null> {
  ...
  if (existingBySlot) {
    usedSlots = new Set(
      Array.from(existingBySlot.entries())
        .filter(([_, def]) => (def as { fieldType: string }).fieldType === fieldType)
        .map(([slot, _]) => slot)
    )
  } else {
    ...
    try {
      ...
      if (isUpdate) {
        ...
        await db
          ...
          .set({ displayName, fieldType, updatedAt: now })
...
export async function deleteTagDefinition(
  ...
  return { tagSlot, displayName: definition.displayName }
}
...
export async function updateTagDefinition(
  ...
  return { ...updated, tagSlot: updated.tagSlot as string }
}

/**
 * Get tag usage with detailed document information
 */
