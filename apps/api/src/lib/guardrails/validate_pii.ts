...
export async function validatePII(input: PIIValidationInput): Promise<PIIValidationResult> {
  ...
  try {
    ...
    return {
      passed: false,
      error: `PII validation failed: ${message}`,
      detectedEntities: [],
    }
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    python.on('close', (code) => {
      ...
      try {
        ...
        resolve({
          passed: false,
          error: `Failed to parse result: ${message}. stdout: ${stdout.substring(0, 200)}`,
          detectedEntities: [],
        })
      }
    })
...
/**
 * All supported PII entity types based on Microsoft Presidio
 */
