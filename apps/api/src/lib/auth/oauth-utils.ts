...
/**
 * Get an OAuth token for a user and provider, refreshing if expired
 */
...
export async function refreshTokenIfNeeded(
  ...
  const accessTokenExpiresAt = credential.accessTokenExpiresAt as Date | null | undefined
  const refreshTokenExpiresAt = credential.refreshTokenExpiresAt as Date | null | undefined
  const providerId = credential.providerId as string
  const refreshToken = credential.refreshToken as string | null | undefined
  const currentAccessToken = credential.accessToken as string | null | undefined
  const userId = credential.userId as string
  const now = new Date()

  const accessTokenNeedsRefresh =
    !!refreshToken &&
    (!currentAccessToken || (accessTokenExpiresAt && accessTokenExpiresAt <= now))
  ...
  if (!shouldRefresh) {
    ...
    return { accessToken: currentAccessToken as string, refreshed: false }
  }

  try {
    const refreshResult = await refreshOAuthToken(providerId, refreshToken!)

    if (!refreshResult) {
      logger.error(`[${requestId}] Failed to refresh token for credential`)
      if (!accessTokenNeedsRefresh && currentAccessToken) {
        logger.info(`[${requestId}] Proactive refresh failed but access token still valid`)
...
/**
 * Get all valid credentials for members of a credential set
 */
