...
export class Memory {
  ...
  private applyContextWindowLimit(messages: Message[], model?: string): Message[] {
    ...
    for (const provider of Object.values(PROVIDER_DEFINITIONS)) {
      if ((provider as any).contextInformationAvailable === false) continue

      const matchesPattern = provider.modelPatternStrings?.some((p: string) => new RegExp(p).test(model))
      const matchesModel = provider.models.some((m: any) => m.id === model)
