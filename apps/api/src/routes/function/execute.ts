...
/** Line count added by the E2B JavaScript async wrapper */
const E2B_JS_WRAPPER_LINES = 3

/** Line count added by the E2B Python def wrapper */
...
/**
 * Lazily load the TypeScript module for AST parsing
 */
...
/**
 * Extract import statements from JavaScript code using TypeScript AST parsing.
 * Returns the extracted imports, the remaining code with imports removed,
 * and the number of lines the imports occupied.
 */
...
function extractEnhancedError(
  error: Record<string, unknown> & { message?: string; name?: string; stack?: string },
...
/**
 * Create a detailed, user-friendly error message from enhanced error info
 */
...
/**
 * Resolve workflow variable references (e.g. {{var_name}}) in user code.
 * Replaces each reference with a safe variable name and populates contextVariables.
 */
function resolveWorkflowVariables(
  code: string,
  workflowVariables: Record<string, Record<string, unknown>>,
  ...
  while ((match = regex.exec(code)) !== null) {
    ...
    const foundVariable = Object.entries(workflowVariables).find(
      ([_, variable]) => normalizeName((variable as Record<string, unknown>).name as string || '') === variableName
    )

    if (!foundVariable) {
      const availableVars = Object.values(workflowVariables)
        .map((v) => (v as Record<string, unknown>).name as string)
        .filter(Boolean)
      throw new Error(
...
/**
 * Resolve environment variable references in user code.
 * Replaces each reference with a safe variable name and populates contextVariables.
 */
...
/**
 * Resolve block reference tag variables (e.g. <blockName.field>) in user code.
 * Replaces each reference with a safe variable name and populates contextVariables.
 */
...
/**
 * Resolves all variable types (workflow, environment, tags) in code.
 *
 ...
 * @param blockData - Block output data for tag resolution
 * @param blockNameMapping - Mapping of block names to IDs
 * @param blockOutputSchemas - Output schemas for blocks
 * @param workflowVariables - Workflow-level variables
 * @param language - Target language for literal formatting
 * @returns Resolved code and the context variables map
 */
function resolveCodeVariables(
  ...
  resolvedCode = resolveWorkflowVariables(
    resolvedCode,
    workflowVariables as Record<string, Record<string, unknown>>,
    contextVariables
  )
 ...
 * This handles the common case where print() or console.log() adds a trailing newline
...
app.post('/execute', async (c) => {
  ...
  try {
    ...
    const enhancedError = extractEnhancedError(err, userCodeStartLine, resolvedCode)
    const userFriendlyErrorMessage = createUserFriendlyErrorMessage(
      enhancedError,
      requestId,
      resolvedCode
