/**
 * Server-side stub for workflow utility functions.
 * Contains pure functions from the web app that do not depend on React/Zustand.
 */
...
export function convertLoopBlockToLoop(
  ...
  loop.forEachItems = (loopBlock.data?.collection as string) || ''
...
export function convertParallelBlockToParallel(
  ...
  const validatedParallelType = validParallelTypes.includes(parallelType as 'collection' | 'count')
    ? parallelType
    : 'collection'

  const distribution =
    validatedParallelType === 'collection' ? (parallelBlock.data?.collection as string) || '' : undefined
