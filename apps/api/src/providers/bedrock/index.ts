    ...
    try {
      ...
      if (currentResponse.output?.message?.content) {
        const structuredOutputCall = currentResponse.output.message.content.find(
          (block: any): block is ContentBlock & { toolUse: ToolUseBlock } =>
        ...
        if (structuredOutputCall && structuredOutputTool) {
          ...
          const textBlocks = currentResponse.output.message.content.filter(
            (block: any): block is ContentBlock & { text: string } => 'text' in block
          )
      ...
      const initialToolUseContentBlocks = (currentResponse.output?.message?.content || []).filter(
        (block: any): block is ContentBlock & { toolUse: ToolUseBlock } => 'toolUse' in block
      )
      const toolUseBlocks = initialToolUseContentBlocks.map((block: any) => ({
      ...
      while (iterationCount < MAX_TOOL_ITERATIONS) {
        ...
        const textContent = textContentBlocks.map((block: any) => block.text).join('\n')
        ...
        const currentToolUses = toolUseContentBlocks.map((block: any) => block.toolUse)
        ...
        const nextToolUseBlocks = nextToolUseContentBlocks.map((block: any) => ({
          name: block.toolUse.name || '',
        }))
