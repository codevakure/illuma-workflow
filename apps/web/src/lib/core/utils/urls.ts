...
export function getBaseUrl(): string {
  const baseUrl = getEnv('NEXT_PUBLIC_APP_URL') || (typeof window !== 'undefined' ? window.location.origin : 'http://localhost:5174')
 ...
 * @returns The domain with port if applicable (e.g., 'localhost:3000' or 'illuma.ai')
 */
export function getBaseDomain(): string {
  try {
    ...
    try {
      ...
      return isProd ? 'illuma.ai' : 'localhost:3000'
    }
  }
}
 ...
 * @returns The email domain (e.g., 'illuma.ai' instead of 'www.illuma.ai')
 */
