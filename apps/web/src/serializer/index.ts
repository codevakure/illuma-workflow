...
export class Serializer {
  ...
  private validateRequiredFieldsBeforeExecution(
    ...
    // Provider credential fields are resolved server-side via BYOK — skip client validation
    ...
    if (currentTool) {
      Object.entries(currentTool.params || {}).forEach(([paramId, paramConfig]) => {
        if (serverManagedFields.has(paramId)) return
    ...
    blockConfig.subBlocks?.forEach((subBlockConfig: SubBlockConfig) => {
      // Skip provider credential fields — resolved server-side via BYOK
      if (serverManagedFields.has(subBlockConfig.id)) {
        return
      }
