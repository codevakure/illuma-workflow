...
function getBlacklistedProviders(): string[] {
  ...
  return (env.BLACKLISTED_PROVIDERS as string).split(',').map((p: string) => p.trim().toLowerCase())
}
...
function getBlacklistedModels(): { models: string[]; prefixes: string[] } {
  ...
  const entries = (env.BLACKLISTED_MODELS as string).split(',').map((m: string) => m.trim().toLowerCase())
