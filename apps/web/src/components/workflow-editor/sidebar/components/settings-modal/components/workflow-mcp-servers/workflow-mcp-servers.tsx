  ...
  const getConfigSnippet = useCallback(
    (client: McpClientType, isPublic: boolean, serverName: string): string => {
      ...
      if (client === 'claude-code') {
        ...
        return `claude mcp add "${safeName}" --url "${mcpServerUrl}" --header "X-API-Key:$ILLUMA_API_KEY"`
      }
      ...
      if (client === 'cursor') {
        const cursorConfig = isPublic
          ? { url: mcpServerUrl }
          : { url: mcpServerUrl, headers: { 'X-API-Key': '$ILLUMA_API_KEY' } }
      ...
      const mcpRemoteArgs = isPublic
        ...
        : ['-y', 'mcp-remote', mcpServerUrl, '--header', 'X-API-Key:$ILLUMA_API_KEY']
                ...
                <div>
                  ...
                  {!server.isPublic && (
                    <p className='mt-[8px] text-[11px] text-[var(--text-muted)]'>
                      Replace $ILLUMA_API_KEY with your API key, or{' '}
                      <button
                        type='button'
            ...
            <div className='flex flex-col gap-[12px]'>
              ...
              <p className='text-[11px] text-[var(--text-muted)]'>
                {editServerIsPublic
                  ...
                  : 'Requests must include your Illuma API key in the X-API-Key header'}
              </p>
            </div>
          </ModalBody>
          <ModalFooter>
