...
export function ApiDeploy({
  ...
  const getBaseEndpoint = () => {
    if (!info) return ''
    return info.endpoint.replace(info.apiKey, '$ILLUMA_API_KEY')
  }
...
response = requests.post(
    "${endpoint}",
    headers={
        "X-API-Key": os.environ.get("ILLUMA_API_KEY"),
...
print(response.json())`
  ...
  method: "POST",
  headers: {
    "X-API-Key": process.env.ILLUMA_API_KEY,
    "Content-Type": "application/json"
  },
...
console.log(job); // Contains jobId and executionId`
      ...
      case 'status':
        switch (language) {
          case 'curl':
            return `curl -H "X-API-Key: $ILLUMA_API_KEY" \\
...
response = requests.get(
    "${baseUrl}/api/jobs/JOB_ID_FROM_EXECUTION",
    headers={"X-API-Key": os.environ.get("ILLUMA_API_KEY")}
)
...
print(status)`
  ...
  "${baseUrl}/api/jobs/JOB_ID_FROM_EXECUTION",
  {
    headers: { "X-API-Key": process.env.ILLUMA_API_KEY }
  }
);
